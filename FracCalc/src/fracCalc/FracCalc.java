 ...
 */
package fracCalc;
 // resubmitted to reduce lines from 400 down to 310, ok'ed by Ms Dreyer. 
...
public static String answerSimplifier (int absnum, int absdem, int finalnum, int finaldem){
	if( finaldem == finalnum){ //simplfies the answer to a reduced or mixed fraction form. 
       return "1";
   ...
   } else if(absnum<absdem&&(finalnum<0||finaldem<0)){
       return "-"+gcf(finalnum,finaldem); 
   } return gcf(finalnum,finaldem);
}
}  